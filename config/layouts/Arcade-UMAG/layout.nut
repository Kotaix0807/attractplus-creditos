...
function put_image(path, x, y, z, w, h)
{
	try{
		local img = fe.add_image(path, x, y, w, h);
		img.zorder = z;                     // capa
		img.preserve_aspect_ratio = true;   // que no se deforme al encajarla
		img.mipmap = true;
		return img;
	}
	catch (e)
...
local field = put_image( "field.png", 0, 0, 0, 456, 336 );
...
local umag = put_image("umag.png", 0, 0, 2147483647, 55, 55);

local dic = put_image( "dic.png", 55, 0, 2147483647, 55, 55 );
